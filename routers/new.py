...
async def cmd_new(message: Message, state: FSMContext):
    ...
    await message.answer(
        text='Enter name of emoji set, which will be used in t.me/addstickers/ URLs. '
             'It can contain only English letters, digits and underscores. '
             'It must begin with a letter, can\'t contain consecutive underscores. '
             'URL will end in _by_emojxbot. It may contain up to 52 characters.'
    )
...
async def incorrect_name_error(message: Message):
    await message.answer(
        text='Incorrect name. Please, make sure your name meets all requirements and try again.'
    )
...
async def correct_title(message: Message, state: FSMContext, bot: Bot):
    ...
    await bot.create_new_sticker_set(
        message.chat.id, f'{data["chosen_emoji_set_name"]}_by_emojxbot',
    ...
    await message.answer(
        text=f'Emoji set was created successfully. '
             f'You can find it at: t.me/addstickers/{data["chosen_emoji_set_name"]}_by_emojxbot'
    )
...
async def incorrect_title_error(message: Message):
    await message.answer(text='Incorrect title. Please, make sure your title meets all requirements and try again.')
