...
async def get_document(message: Message, state: FSMContext, bot: Bot):
    ...
    resize_and_overlay('images/text.png', f'images/liquidated_{message.from_user.id}.png', f'images/result_liquidated_{message.from_user.id}.png')
    await state.set_state(Liquidated.done)
    await bot.send_photo(chat_id=message.from_user.id, photo=FSInputFile(f'images/result_liquidated_{message.from_user.id}.png'))
