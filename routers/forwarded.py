...
from modules.forwarded_from import resize_and_overlay
...
async def get_document(message: Message, state: FSMContext, bot: Bot):
    ...
    resize_and_overlay(path, f'images/forwarded_from_{message.from_user.id}.gif')
    await state.set_state(Forwarded.done)
    await bot.send_animation(
        chat_id=message.from_user.id, animation=FSInputFile(f'images/forwarded_from_{message.from_user.id}.gif')
    )
